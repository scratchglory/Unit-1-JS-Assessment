...
function getVehiclesCostInCreditsSumTotal(character) {
  ...
  for (let i = 0; vehicles.length > i; i++) {
    let ship = vehicles[i].cost_in_credits;
    costs += ship;
  }
  return costs;
}
...
function getStarshipPassengerAndCrewSumTotal(character) {
  // TODO: Add your code here.
  let capacity = character.starships.reduce((a, b) => {
    let crewCap = b.crew;
    return a + crewCap + b.passengers;
  }, 0);
  return capacity;
...
function getCargoCapacityTotal(character) {
  // TODO: Add your code here.
  const x = character.vehicles;
  const y = character.starships;

  let totalCargo = [];
  for (let i = 0; x.length > i; i++) {
    num1 = Number(x[i].cargo_capacity);
    totalCargo.push(num1);
  }
  for (let i = 0; y.length > i; i++) {
    num2 = Number(y[i].cargo_capacity);
    totalCargo.push(num2);
  }

  const reducer = (acc, currentValue) => acc + currentValue;
  return totalCargo.reduce(reducer);
}
// ***Want to know a simpler way of solving
